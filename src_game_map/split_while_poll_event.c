...
void cond_key_l_r(sfEvent event, sfView *view, anime_t *anim)
{
    ...
    if (event.key.code == sfKeyRight) {
        if (handle_collision(anim->pos.x + 20, anim->pos.y) == 0)
            return;
        if (handle_collision(anim->pos.x + 20, anim->pos.y - 28) == 0)
        return;
    ...
    if (event.key.code == sfKeyLeft) {
        if (handle_collision(anim->pos.x, anim->pos.y) == 0)
            return;
        if (handle_collision(anim->pos.x, anim->pos.y - 28) == 0)
        return;
...
void cond_key_u_d(sfEvent event, sfView *view, anime_t *anim)
{
    ...
    if (event.key.code == sfKeyUp) {
        if (handle_collision(anim->pos.x + 22, anim->pos.y - 28) == 0)
            return;
        if (handle_collision(anim->pos.x + 2, anim->pos.y - 28) == 0)
        return;
